...
def parse_date_arg(s):
    """Parse YYYY-MM-DD, 'today', 'yesterday', '3d' (N days ago), '6h' (N hours ago) into datetime (start of day UTC)."""
    now = datetime.now(timezone.utc)
    if s == "today":
        return now.replace(hour=0, minute=0, second=0, microsecond=0)
    if s == "yesterday":
        return (now - timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
    # Relative: "3d" = 3 days ago, "6h" = 6 hours ago, "30m" = 30 min ago
...
def parse_date_end(s):
    """Parse end date -> end of day (23:59:59 UTC)."""
    d = parse_date_arg(s)
    # For relative times (hours/minutes), keep the exact time; for dates, go end of day
    m = re.match(r'^(\d+)([hms])$', s) if isinstance(s, str) else None
    if m:
        return d  # exact timestamp for relative hour/minute/second
    return d.replace(hour=23, minute=59, second=59, microsecond=999999)


def build_time_filter(since, until):
    """Return (where_clause, params) for time range on received_at (unix ms)."""
    clauses = []
    params = []
    if since:
...
def get_category(app_package, app_name):
    """Determine notification category from app package or name."""
...
def cmd_report(args):
    ...
    if args.app:
        # Filter by app name (case-insensitive LIKE)
    ...
    # Total count
    count = conn.execute(
        f"SELECT COUNT(*) as cnt FROM notifications WHERE 1=1{where}{app_clause}",
        params
    ).fetchone()["cnt"]

    # Per-app breakdown with categories
    rows = conn.execute(
        f"""SELECT app_package, app_name, COUNT(*) as cnt,
                   MIN(received_at) as first_seen, MAX(received_at) as last_seen
            FROM notifications
            WHERE 1=1{where}{app_clause}
    ...
    for cat in ["messenger", "dev", "work", "email", "social", "other"]:
        ...
        for r in items:
            ...
            print(f"    {r['app_name']:25s}  {r['cnt']:4d} notif.  [{first.strftime('%H:%M')} → {last.strftime('%H:%M')}]")
        print()

    if args.app:
        # Show actual notification content for the specific app
        print(f"\n🔍 Details for «{args.app}»:")
        conn = get_conn()
        detail_where = where + app_clause
        details = conn.execute(
            f"SELECT id, app_name, title, text, sub_text, timestamp_iso "
            f"FROM notifications WHERE 1=1{detail_where} ORDER BY received_at DESC LIMIT 20",
            params
        ).fetchall()
        conn.close()
        for d in details:
...
def cmd_by_app(args):
    ...
    rows = conn.execute(
        f"""SELECT app_name, app_package, COUNT(*) as cnt
            FROM notifications
            WHERE 1=1{where}
            GROUP BY app_package, app_name
            ORDER BY cnt DESC""",
...
def cmd_search(args):
    ...
    rows = conn.execute(
        """SELECT id, app_name, title, text, timestamp_iso, received_at
           FROM notifications
           WHERE title LIKE ? OR text LIKE ? OR app_name LIKE ?
...
def cmd_stats(args):
    ...
    # Overall stats
    total = conn.execute(
        f"SELECT COUNT(*) as c FROM notifications WHERE 1=1{where}", params
    ).fetchone()["c"]

    apps = conn.execute(
        f"SELECT COUNT(DISTINCT app_package) as c FROM notifications WHERE 1=1{where}", params
    ).fetchone()["c"]

    # Per hour breakdown for the period to find peak hours
    peak = conn.execute(
        f"""SELECT CAST((received_at / 3600000) % 24 AS INTEGER) as hour,
                   COUNT(*) as cnt
            FROM notifications
            WHERE 1=1{where}
            GROUP BY hour
    ...
    # First and last
    bounds = conn.execute(
        f"""SELECT MIN(received_at) as first_ts, MAX(received_at) as last_ts
            FROM notifications
            WHERE 1=1{where}""",
        params
    ).fetchone()
    ...
    print(f"📊 Notification statistics | {period}")
...
def cmd_export(args):
    ...
    where, params = build_time_filter(args.since, args.until)
    rows = conn.execute(
        f"SELECT * FROM notifications WHERE 1=1{where} ORDER BY received_at ASC",
        params
    ).fetchall()
    conn.close()
    ...
    if fmt == "json":
        ...
        for d in data:
            ...
            d.pop("raw_data", None)
            out.append(d)
        (out_dir / "notifications.json").write_text(
        ...
        for cat, items in by_cat.items():
            ...
            md = [f"# {label}\n"]
            total = len(items)
            md.append(f"Total: {total} notifications\n")
            md.append("| Time | App | Title | Text |")
            md.append("|------|-----|-------|------|")

            for r in items[:100]:  # max 100 per file
                dt = datetime.fromtimestamp(r["received_at"] / 1000, tz=timezone.utc)
                title = r["title"].replace("|", "\\|")[:60]
                text = r["text"].replace("\n", " ").replace("|", "\\|")[:100]
                md.append(f"| {dt.strftime('%H:%M:%S')} | {r['app_name']} | {title} | {text} |")

            if len(items) > 100:
                md.append(f"\n*Showing first 100 of {total}*")

            fname = out_dir / f"{cat}.md"
            fname.write_text("\n".join(md), encoding="utf-8")
            print(f"✓ {fname.name} — {total} records")
...
def main():
    parser = argparse.ArgumentParser(
        description="Notification Webhook Analyzer — query, analyse, and report"
    )
    ...
    p.add_argument("--since", default=None, help="Start date (YYYY-MM-DD, today, yesterday, 7d, 6h)")
    p.add_argument("--until", default=None, help="End date (YYYY-MM-DD, today, yesterday)")
    p.add_argument("--app", default=None, help="Filter by app name")
    ...
    # search
    p = sub.add_parser("search", help="Full-text search in notifications")
    p.add_argument("QUERY", help="Search term")
    p.set_defaults(func=cmd_search)

    # stats
    p = sub.add_parser("stats", help="Summary statistics")
    p.add_argument("--since", default=None, help="Start date (YYYY-MM-DD, today, yesterday, 7d)")
    p.add_argument("--until", default=None, help="End date (YYYY-MM-DD, today, yesterday)")
    p.set_defaults(func=cmd_stats)

    # export
    p = sub.add_parser("export", help="Export notifications to files")
    p.add_argument("--since", default=None)
    p.add_argument("--until", default=None)
