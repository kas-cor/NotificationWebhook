...
- 🌗 **Material3 Design** — светлая и тёмная тема (автоматически под систему, и ручной выбор)
...
- 🌐 **Локализация** — язык системы / русский / английский (сохраняется, применяется при повторном открытии Activity)
- 🎨 **Тема** — система / светлая / тёмная (реактивно, без перезапуска)
- 🧹 **Автосмахивание промо** — если сервер классифицирует уведомление как промо/акцию, приложение смахивает его (чистый док)
- ➕ **Создать правило из истории** — у каждой записи истории кнопка «+», превращает отправителя в правило исключения в один тап
...
NotificationListenerService ← системный bind через BIND_NOTIFICATION_LISTENER_SERVICE
        ...
        │    + опциональный запрос классификации промо + опрос статуса
        ▼
   Webhook HTTP POST
...
| `MainActivity.kt` | Compose-хост: применяет сохранённый язык, держит реактивное состояние темы/языка, делает recreate при смене языка, передаёт колбэки в `MainScreen` |
| `ui/MainScreen.kt`, `ui/*Tab.kt` | Compose UI: Главная (статус/webhook), Исключения (приложения + правила), История, Настройки (переключатели, локализация, тема, о приложении) |
...
| `AppPrefs.kt` | SharedPreferences singleton (потокобезопасный, зашифрованный AES256-GCM) |

## Вкладка «Настройки»

- **Пересылать уведомления** — основной переключатель; требует доступа к уведомлениям для включения
- **Пропускать ongoing** — музыка, навигация, системные уведомления
- **Автосмахивание промо** — переключатель классификации (по умолчанию включено)
- **Язык** — язык системы / русский / английский
- **Тема** — система / светлая / тёмная
- **О приложении** — версия (из `BuildConfig.VERSION_NAME`), проверка обновлений (GitHub API), ссылка на репозиторий

## Локализация

Все строки вынесены в `res/values/strings.xml` (английский по умолчанию) и `res/values-ru/strings.xml`. Выбор языка сохраняется в `AppPrefs` и применяется при повторном открытии Activity; по умолчанию используется язык системы, при отсутствии поддержки — английский.
...
- **Тёмная тема** — автоматически под систему или ручной выбор (светлая/тёмная)
- **Нижняя навигация, 4 вкладки:** Главная (статус, webhook URL) / Исключения (приложения + правила) / История / Настройки (переключатели, локализация, тема, о приложении)
- **Bearer token** — поле ввода под webhook URL с переключателем видимости пароля
- **Локализация** — EN/ru/системная; тема система/светлая/тёмная
